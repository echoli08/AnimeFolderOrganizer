...
這是一個基於 WPF (.NET 9) 開發的桌面應用程式，利用 AI 服務（Google Gemini / 自訂 OpenAI 相容 API）的語言理解能力，協助使用者智慧辨識並自動整理動畫資料夾名稱。
...
*   **AI 智慧辨識**：整合 Google Gemini / 自訂 OpenAI 相容 API，精準分析資料夾名稱中的動畫資訊。
*   **批量處理**：一次掃描並處理多個資料夾。
...
*   **sub_share 字幕資料庫**：支援本機資料庫更新與關鍵字匹配，命中時提供多語系標題與字幕狀態。
*   **字幕下載 / 上傳包**：命中 sub_share 時可右鍵下載字幕，或建立上傳包供手動上傳。
...
*   **AI 服務**：Google Gemini API / 自訂 OpenAI 相容 API
...
3.  **API Key**：
    *   Google Gemini 或自訂 OpenAI 相容 API 其一（必要）。
    *   TMDB API Key（用於官方名稱補正，可選）。
    *   Gemini 可至 [Google AI Studio](https://aistudio.google.com/app/apikey) 申請。
...
3.  **掃描與辨識**：
    ...
    *   若 sub_share 命中，清單會顯示 sub_share 與字幕狀態。
...
6.  **sub_share 資料庫**：
    *   設定頁提供 sub_share 資料庫更新與匯入。
    *   資料庫位置為執行檔同目錄 `subshare/db.xml`。
